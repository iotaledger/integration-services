...
export const fetchAuth = async (): Promise<any> => {
	const identityBuffer = fs.readFileSync('./src/config/LogCreatorIdentity.json');
	...
	const res = await logCreatorClient.get(url);
	if (res.status !== 200) {
	...
	const response = await logCreatorClient.post(`${Config.baseUrl}/authentication/prove-ownership/${identity.doc.id}${apiKey}`, JSON.stringify({ signedNonce }), {
		method: 'post',
