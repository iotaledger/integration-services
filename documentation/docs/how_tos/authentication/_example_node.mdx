...
### Required Packages{#required-packages-node}
...
## Authentication Workflow{#authentication-workflow-node}

### 1. Request a Nonce{#1-request-a-nonce-node}
...
### 2. Hash the Nonce{#2--hash-the-nonce-node}
...
### 3. Sign the Hashed Nonce{#3--sign-the-hashed-nonce-node}
...
### 4. Request the JWT{#4--request-the-jwt-node}
...
### 5. Set the JWT as Axios Header{#5--set-the-jwt-as-axios-header-node}
...
##  Putting It All Together{#putting-it-all-together-node}
