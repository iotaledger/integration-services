...
1. Create a new Identity (`userIdentity`).
2. Create a credential for that identity (`userCredential`).
3. Verify the generated credential (`await identity.checkCredential(userCredential)`).
