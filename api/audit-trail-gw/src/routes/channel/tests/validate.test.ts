...
describe('test validate route', () => {
	...
	it('should throw an error since no streams sub is found', async () => {
		...
		expect(getSubSpy).toHaveBeenCalledWith('someteststate', true, StreamsConfigMock.statePassword);
	...
	it('should validate the logs', async () => {
		...
		expect(importSubscriptionSpy).toHaveBeenCalledWith('someteststate', true, StreamsConfigMock.statePassword);
