...
describe('test request subscription route', () => {
	...
	it('should return bad request since no channelAddress is provided', async () => {
		spyOn(subscriptionService, 'getSubscription').mockReturnValue({}); // already a subscription is found!
		const req: any = {
	...
	it('should not create a subscription since publicKey already used', async () => {
		...
		spyOn(subscriptionService, 'getSubscription').mockReturnValue(null);
		const getSubscriptionByPublicKeySpy = jest.spyOn(subscriptionService, 'getSubscriptionByPublicKey').mockReturnValue(foundSubscription); // found some subscription so should return an error
		const exportSubscriptionSpy = jest.spyOn(streamsService, 'exportSubscription').mockReturnValue('teststate');

		const requestSubscriptionSpy = jest.spyOn(streamsService, 'requestSubscription').mockReturnValue({
			subscriber: null,
			subscriptionLink: 'testlink',
			publicKey: 'testpublickey',
			seed: 'testseed'
